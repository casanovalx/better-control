...
def parse_arguments() -> argparse.Namespace:
    ...
    Returns:
        argparse.Namespace: a namespace of argparse
    """
...
def get_battery_info(device: str) -> str:
...
def detect_peripheral_battery() -> typing.Tuple:
...
def check_dependency(
    command: str, name: str, install_instructions: str
) -> typing.Optional[str]:
...
def check_all_dependencies() -> bool:
    ...
    Returns:
        bool: returns false if the are dependencies missing, or return true
    """
...
def load_settings() -> typing.Dict[str, typing.Any]:
    """Returns the setting from SETTINGS_FILE

    Returns:
        typing.Dict[str, typing.Any]: a json-like object
    """
...
def save_settings(settings: typing.Dict[str, typing.Any]) -> None:
    """Save settings to SETTINGS_FILE

    Args:
        settings (typing.Dict[str, typing.Any]): setting object
    """
    try:
...
class BatteryTab(Gtk.Box):
        ...
        if mode_string in self.power_modes.values():
            for key, value in self.power_modes.items():
                if value == mode_string:
                    self.power_mode_dropdown.set_active(
                        list(self.power_modes.keys()).index(key)
                    )
                    break
        else:
            self.power_mode_dropdown.set_active(
    ...
    def refresh_battery_info(self, button=None):
        ...
        for path in battery_paths:
            ...
            # Capacity health
            try:
                if os.path.exists(f"{path}/energy_full") and os.path.exists(
                    f"{path}/energy_full_design"
                ):
            ...
            try:
                if (
                    os.path.exists(f"{path}/energy_now")
                    and os.path.exists(f"{path}/power_now")
                    and battery_info.get("State") == "Discharging"
                ):
...
class bettercontrol(Gtk.Window):
    def __init__(self, args):
        ...
        if "hyprland" in os.environ.get("XDG_CURRENT_DESKTOP", "").lower():
            subprocess.run(
                ["hyprctl", "keyword", "windowrulev2", "float,class:^(control)$"]
            )
        ...
        self.original_tab_positions = (
            self.original_tab_positions
            if hasattr(self, "original_tab_positions")
            else {}
        )
        ...
        settings_icon: Gtk.Image = Gtk.Image.new_from_icon_name(
        ...
        if self.tab_visibility.get("Wi-Fi", True):
            self.notebook.append_page(
                wifi_box, self.create_tab_label_with_icon("Wi-Fi")
            )
        ...
        bt_icon = Gtk.Image.new_from_icon_name(
        ...
        self.bt_status_switch.connect(
            "notify::active", self.on_bluetooth_switch_toggled
        )
        ...
        if self.tab_visibility.get("Bluetooth", True):
            self.notebook.append_page(
                scrolled_bt, self.create_tab_label_with_icon("Bluetooth")
            )
        ...
        self.sink_store = Gtk.ListStore(
            str, str, str
        )  # Device ID, Display Name, Icon Name
        ...
        # Replace ComboBoxText with ComboBox for icons
        self.source_store = Gtk.ListStore(
            str, str, str
        )  # Device ID, Display Name, Icon Name
        ...
        input_refresh_icon = Gtk.Image.new_from_icon_name(
            "view-refresh-symbolic", Gtk.IconSize.BUTTON
        )
        ...
        if self.tab_visibility.get("Volume", True):
            self.notebook.append_page(
                scrolled_volume, self.create_tab_label_with_icon("Volume")
            )
        ...
        if self.tab_visibility.get("Display", True):
            self.notebook.append_page(
                display_placeholder, self.create_tab_label_with_icon("Display")
            )
        ...
        if self.tab_visibility.get("Battery", True):
            self.notebook.append_page(
                self.battery_tab, self.create_tab_label_with_icon("Battery")
            )
        ...
        if args.volume:
            ...
            initial_tab = next(
                iter(self.tabs)
            )  # Default to Volume if no argument is provided
    ...
    def set_blue_light(self, value):
        ...
        if shutil.which("gammastep"):
            ...
            self.show_error_dialog(
                "gammastep is missing. please check our github page to see all dependencies and install them"
            )
    ...
    def set_bluelight_filter(self, scale):
        ...
        subprocess.Popen(
            ["gammastep", "-O", str(self.temperature)],
            stdout=subprocess.DEVNULL,
            stderr=subprocess.DEVNULL,
        )
            ...
            for sink_line in short_sinks:
                ...
                if len(parts) >= 2:
                    ...
                    if sink_id in sink_indices:
                        ...
                        section = sink_sections[
                            section_idx + 1
                        ]  # +1 because we skipped the first empty section
                        lines = section.split("\n")
            ...
            if not friendly_names:
                for section in sink_sections[1:]:
                    ...
                    if (
                        sink_id is not None
                        and description
                        and len(self.sink_list) > int(sink_id)
                    ):
    ...
    def update_source_list(self, button=None):
        ...
        try:
            ...
            for source_line in short_sources:
                ...
                if len(parts) >= 2:
                    ...
                    if source_id in source_indices:
                        ...
                        if (
                            "monitor" in source_name.lower()
                            and not source_name.lower().startswith("monitor")
                        ):
                            continue
                        ...
                        if description:
                            ...
                            if source_name.startswith("alsa_input."):
                                humanized = (
                                    source_name.replace("alsa_input.", "")
                                    .replace("_", " ")
                                    .replace(".", " ")
                                    .title()
                                )
            ...
            for source in self.source_list:
                ...
                if "monitor" in source.lower() and not source.lower().startswith(
                    "monitor"
                ):
                    continue
                ...
                if (
                    display_name in friendly_names.values()
                ):  # Only add if we have a friendly name
            ...
            if current_source in self.source_list:
                current_display_name = friendly_names.get(
    ...
    def add_tabs(self):
        ...
        for tab_name, init_method in tab_definitions.items():
            if self.tab_visibility.get(tab_name, True):
                ...
                self.notebook.append_page(
                    placeholder, self.create_tab_label_with_icon(tab_name)
                )
    ...
    def refresh_display(self, button=None):
        ...
        # Blue light slider
        self.blue_light_slider = Gtk.Scale.new_with_range(
    ...
    def populate_settings_tab(self):
        ...
        instruction_label = Gtk.Label(
            label="Use the checkboxes to show/hide tabs and the arrows to change tab order."
        )
        ...
        for tab_name in tab_list:
            ...
            icon_name = tab_icons.get(
                tab_name, "applications-system-symbolic"
            ...
            up_image = Gtk.Image.new_from_icon_name(
                "go-up-symbolic", Gtk.IconSize.SMALL_TOOLBAR
            )
            ...
            down_image = Gtk.Image.new_from_icon_name(
                "go-down-symbolic", Gtk.IconSize.SMALL_TOOLBAR
            )
        ...
        if button.get_active():
            if page_num == -1:
                ...
                if tab_name in self.original_tab_positions:
                    ...
                    position = min(
                        self.original_tab_positions[tab_name],
    ...
    def save_settings(self):
        ...
        try:
            ...
            settings_data = {"visibility": self.tab_visibility, "positions": {}}
            ...
            for tab_name in self.tabs.keys():
                ...
                if page_num != -1:
                    ...
                    settings_data["positions"][tab_name] = self.original_tab_positions[
                        tab_name
                    ]
                    ...
                    try:
                        with open(SETTINGS_FILE, "r") as f:
                            ...
                            if isinstance(data, dict) and not (
                                "visibility" in data and "positions" in data
                            ):
    ...
    def enable_bluetooth(self, button):
        ...
        try:
            ...
            rfkill_status = subprocess.run(
                ["rfkill", "list", "bluetooth"], capture_output=True, text=True
            )
            ...
            if "bluetooth" not in rfkill_status.stdout:
                ...
                result = subprocess.run(
                    ["rfkill", "unblock", "bluetooth"], capture_output=True, text=True
                )
                    ...
                    for _ in range(3):
                        ...
                        if bt_status == "active":
                            ...
                            if "Powered: yes" in check_result.stdout:
                                ...
                                print(
                                    "Warning: Service is active but Bluetooth is not powered on"
                                )
        ...
        try:
            ...
            test_result = subprocess.run(
                ["systemctl", "status", "bluetooth"], capture_output=True, text=True
            )
            ...
            if "Access denied" in test_result.stderr:
                ...
                start_result = subprocess.run(
                    ["systemctl", "start", "bluetooth"], capture_output=True, text=True
                )
    ...
    def disable_bluetooth(self, button):
        ...
        try:
            ...
            if "bluetooth" not in rfkill_status.stdout:
                ...
                result = subprocess.run(
                    ["rfkill", "block", "bluetooth"], capture_output=True, text=True
                )
        ...
        try:
            ...
            if "Access denied" in test_result.stderr:
                ...
                stop_result = subprocess.run(
                    ["systemctl", "stop", "bluetooth"], capture_output=True, text=True
                )
    ...
    def connect_bluetooth_device_selected(self, button):
        ...
        def connect_thread():
            try:
                ...
                GLib.idle_add(
                    lambda: self.bt_status_label.set_text(f"Connected to {device_name}")
                )
                GLib.idle_add(self.refresh_bluetooth, None)
            except Exception as e:
                GLib.idle_add(
                    lambda: self.bt_status_label.set_text(
    ...
    def forget_bluetooth_device_selected(self, button):
        ...
        def forget_thread():
            try:
                ...
                GLib.idle_add(
                    lambda: self.bt_status_label.set_text(f"Removed {device_name}")
                )
                GLib.idle_add(self.refresh_bluetooth, None)
            except Exception as e:
                GLib.idle_add(
                    lambda: self.bt_status_label.set_text(f"Removal failed: {str(e)}")
                )
    ...
    def on_tab_switch(self, notebook, tab, page_num):
        ...
        if tab_label == "Bluetooth":
            if not shutil.which("bluetoothctl"):
                self.show_error_dialog(
                    "bluetoothctl is missing. Please check our GitHub page to see all dependencies and install them."
                )
            else:
            ...
            if not shutil.which("nmcli"):
                self.show_error_dialog(
                    "NetworkManager (nmcli) is missing. Please check our GitHub page to see all dependencies and install them."
                )
        ...
        elif tab_label in ["Volume", "Application Volume"] and not shutil.which(
            "pactl"
        ):
            self.show_error_dialog(
                "pactl is missing. Please check our GitHub page to see all dependencies and install them."
            ...
            self.show_error_dialog(
                "powerprofilesctl is missing. Please check our GitHub page to see all dependencies and install them."
            )
    ...
    def on_tab_switched(self, notebook, page, page_num):
        ...
        elif tab_label == "Bluetooth" and not self._tabs_initialized.get(
            "Bluetooth", False
        ):
        ...
        elif tab_label == "Battery" and not self._tabs_initialized.get(
            "Battery", False
        ):
            self.initialize_battery_tab()
        elif tab_label == "Display" and not self._tabs_initialized.get(
            "Display", False
        ):
        ...
        try:
            ...
            for sink_input in sink_inputs:
                ...
                app_icon = Gtk.Image.new_from_icon_name(
                    final_icon_name, Gtk.IconSize.LARGE_TOOLBAR
                )
                ...
                scale.connect(
                    "value-changed", self.set_app_volume, app_name, media_name
                )
    ...
    def get_app_volume(self, sink_input_id):
        ...
        try:
            output = subprocess.getoutput(
                f"pactl get-sink-input-volume {sink_input_id}"
            )
            if "No such entity" in output or "No valid command specified" in output:
                raise ValueError(
                    f"Sink input {sink_input_id} no longer exists or is invalid."
                )

            volume_parts = output.split("/")
            if len(volume_parts) < 2:
                raise ValueError(
                    f"Unexpected output format for sink input {sink_input_id}: {output}"
                )
        ...
        try:
            ...
            for sink_input in sink_inputs:
                ...
                if current_app_name == app_name and current_media_name == media_name:
                    ...
                    if result.returncode != 0:
                        print(
                            f"Failed to set volume for sink input {sink_input_id}: {result.stderr}"
                        )
                    else:
                        print(
                            f"Volume set to {new_volume}% for {app_name} - {media_name} (sink input {sink_input_id})"
                        )
                    return

            print(
                f"Failed to find sink input for application: {app_name} - {media_name}"
            )
    ...
    def refresh_app_volume_realtime(self):
        ...
        try:
            ...
            if (
                hasattr(self, "previous_sink_inputs")
                and self.previous_sink_inputs == current_sink_inputs
            ):
                return True
    ...
    def move_tab_up(self, button, tab_name):
        ...
        self.notebook.set_current_page(
            current_active_tab
            if current_active_tab != current_position
            else new_position
        )
